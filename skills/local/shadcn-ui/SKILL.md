...
the reusable clean-light CMS/admin visual baseline, load `admin-product-ux`.
...
- **Theme toggle:** three-state (Light / Dark / System) per `design-taste`
  §4.2.1. `next-themes` handles FOUC in Next; outside Next write the inline
  `<head>` script yourself. Set `color-scheme` per theme in CSS.
