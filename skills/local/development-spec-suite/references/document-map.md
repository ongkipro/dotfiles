...
Start with the pack's `02-PRD.md` and add only triggered artifacts. After requirements/contracts are accepted, `prd-taskbreaker` writes the repository's root `TASKS.md`, the sole execution queue; never a `TASKS.md` beside `02-PRD.md`. Validate them together with `check-traceability.py <pack> --tasks <repo>/TASKS.md`: only its `T-*` task blocks feed the task rules (`TASK*`, `TRACE001`); standalone `TASK-*` entries are ignored, and without `--tasks` only the pack is checked. When no suite pack exists, `prd-taskbreaker` keeps its standalone root-level `PRD.md`/`TASKS.md` convention. Never maintain both `PRD.md` and `02-PRD.md` as competing canonical requirements.
