...
- Recovery paths use Antigravity, Codex, Anthropic, and 9Router. 9Router (`9router-fantastico`) is the **last** step of the `slow`, `plan`, and `default` chains, not an intermediate buffer: it is a backup for when the three primary pools are all unavailable, and it is deliberately the least-preferred hop. Two reasons. Its own allowance is small, so putting it first would burn the reserve on the most frequent failures rather than keeping it for the ones with nowhere else to go. And its `ag/*` models draw on the same Antigravity account the primary pool already uses, so routing an Antigravity model through 9Router adds no capacity at all — only the `cx/*` models represent genuinely separate headroom, and even those are limited. Reach for it last, not early.
