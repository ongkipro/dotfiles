...
- Recovery paths use the three directly-connected providers only: **Antigravity, Codex, and Anthropic**. Every role and every fallback hop resolves to one of those three. 9Router was removed from routing on 2026-08-16: it is a re-seller, and a hop through it either re-spends an allowance the primary pool is already spending (`ag/*` is the same Antigravity account) or adds a transport and a failure mode between the session and a model the setup can reach directly. Every chain still crosses vendors without it, so nothing was lost. The provider definition stays in `models.yml` — that is what makes 9Router reachable at all — so it remains available deliberately, through an overlay, rather than silently in the normal path.
