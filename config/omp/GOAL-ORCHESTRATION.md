...
4. Route bounded work by capability: discovery/research; implementation and data;
   visual design for new surfaces, material redesigns, reference-driven UI, or unsettled
   UX; correctness review; and sensitive review for authorization, payments, migrations,
   secrets, cryptography, tenant
...
5. Delegate independent slices large enough to justify worker startup and
   integration. Keep small adjacent fixes and bounded maintenance inside an accepted
   visual system local unless a capability or separate review requires another agent;
   browser-visible work still gets proportional `ui-validation`. Never run children with a shared
