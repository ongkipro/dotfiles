...
For an explicit OMP goal in a repository with `TASKS.md`, read `~/dotfiles/config/omp/GOAL-ORCHESTRATION.md` before dispatch. The goal is the destination; `TASKS.md` is the canonical execution queue, and repository evidence decides completion. The parent owns integration and verification; Terra is the normal cost-aware parent, while Fable is the native plan model and `@orchestrator` an explicit hard-session alias, never an automatic parent switch.
Capability lanes are strict: `sonic` / Gemini handles trivial work; `scout`/`librarian` discovery and research use Terra; `task` / Sol: all development and database implementation; `designer` / Gemini 3.7 Flash at its top `high` level: the default visual lane, escalating to `vision` / Opus for a material redesign or a critique that decides direction; `reviewer` / Opus: independent correctness review; `security-reviewer` / Opus XHigh: sensitive review. Non-trivial cross-module R2 gets Opus review; R3/R4 and sensitive work require independent Opus review plus delivery-ledger approval. Approval gates remain load-bearing. Committing and pushing remain explicit user actions, never automatic Goal Mode consequences.
...
For repositories using `delivery-ledger`, every R1-R4 run must capture its base HEAD and pre-existing dirty paths, declare its allowed change surface, and pass the final task boundary before `DONE`. Unexplained out-of-scope, protected, higher-risk, or touched user changes must fail or require explicit expansion, verification, and independent review. Canonical mechanics and limitations: `~/dotfiles/docs/task-change-boundary.md`.

A `PASS` run whose boundary check observed a changed `.tsx/.jsx/.vue/.svelte/.astro/.css/.scss/.html` file — component tests and stories excluded — must record `ui-validation` in its skill attribution, or `delivery-skill-usage verify` refuses it. The trigger is the boundary check's own observed paths, never the agent's account of what it did: `skillsUsed` is self-declared, and an agent that skipped browser evidence is precisely the one that would not report skipping it. A green build is not evidence that a rendered surface works.
