...
**Why:** decided to hold ("pending dulu") on 2026-06-03 after finding Pipedream needs multi-header/OAuth. **How to apply:** if user wants to add a provider, steer to single-static-header/self-auth-URL ones (Zapier) unless we first build multi-header + token-refresh support. See [[project_repliz_integration_location]].
