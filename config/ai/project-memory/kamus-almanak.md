...
- Commit **without** the `Co-Authored-By` trailer. The rule is written down in that repo (`memory/no-ai-commit-trailer.md`) but its scope is **every repository the user owns**, not Kamus alone — it now also lives in `AGENTS.md`, which every CLI loads on every request. Filing it here as a Kamus note is what once caused it to be honoured in Kamus and missed in dotfiles.
