...
TokoΦ (`ongkipro/tokophi`, **PRIVATE lagi sejak 2026-09-09** (perintah eksplisit owner — flip KEENAM, hari yang sama dengan flip kelima). Riwayat: private → public 27–28 Agu → private 28 Agu → public 1–4 Sep → private 4 Sep → public 9 Sep → private 9 Sep. Tuasnya selalu menit GitHub Actions (public gratis, private dibebankan), dan CI sekarang mem-build empat surface tiap PR di atas suite 166 berkas. Isi repo terindeks publik sejak jendela 27–28 Agu, dan sejak 9 Sep juga lima komentar audit issue + issue #388 yang diposting saat publik — fase private menutup akses BARU, tidak menarik yang sudah tersebar. Aturan tetap: JANGAN flip tanpa perintah eksplisit baru — **selalu verifikasi dengan `gh api repos/ongkipro/tokophi --jq .visibility`, jangan percaya catatan ini** (nilainya sudah berubah enam kali) dianggap terindeks publik sejak jendela 27–28 Aug) — Shopify-style commerce SaaS for Indonesia. Monorepo: `apps/{admin,super-admin,storefront}`, `packages/{db,data,lib,ui,sections}`, `specs/`.
...
- **PULL DULU, SELALU — GitHub `main` = kebenaran, lokal hanya cermin** (aturan owner 2026-09-25). Owner mengembangkan dari beberapa device; GitHub `main` sudah presisi dengan production Coolify (push ke `main` = auto-deploy). Awal setiap sesi tokophi, sebelum menyentuh apa pun:
  1. `git fetch origin && git status` → `git pull --ff-only origin main`. Kalau file lokal yang di-track bentrok dan bukan kerjaan sesi ini: owner memilih **GitHub menang** — simpan patch-nya dulu ke `~/Documents/work/notes/tokophi-local-backup-<tanggal>/`, lalu samakan ke HEAD. **Jangan pernah `git stash`** (global, milik agent lain). File untracked lokal → **pindahkan** ke folder backup itu, jangan dihapus.
  2. DB lokal ikut dipresisikan: `docker compose up -d db` → `npm run migrate -w @tokophi/db` (jumlah baris `drizzle.__drizzle_migrations` + stempel max harus sama dengan production) → data demo pakai `npm run seed:recent -w @tokophi/db` (hanya toko demo `batiknusantara`, idempoten). **Fokus akun toko = akun demo saja; jangan `SEED_FORCE`.**
  3. Paritas production dicek read-only: tag image keempat container Coolify = sha commit (`specs/docs/DEPLOY.md` → "artefact check"), dan count migration prod = lokal.
  - Pengembangan: branch/worktree `feat/...` dari `main` terbaru → migration ditulis tangan dengan stempel di atas max production → `npm test` + `test:db` + `verify-rls` + build → PR → merge satu per satu → verifikasi deploy 4/4 dua kali poll. `main` lokal tidak pernah di-commit langsung.
  - **Why:** lokal pernah tertinggal ~30 commit + 7 migration dan membawa edit Codex yang tak ter-commit; kerja di atas lokal basi = deploy yang tidak presisi dengan live.
