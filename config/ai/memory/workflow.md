...
- Dotfiles sync: saat ada update di GitHub (`ongkipro/dotfiles`), pull dan deploy ke lokal (terutama memory `~/.config/ai/memory/`). Sebaliknya, saat ada perubahan lokal di `~/dotfiles` yang perlu disimpan, default-kan `dotsync sync` (review → commit → push opsional) agar aman lintas Linux/macOS; `dotpush` tetap boleh untuk fast-path manual. Pastikan presisi — jangan rusak pola yang ada.
...
- Skill tooling (`skill-update`, `skill-list`, `skill-new`, `skill-open`, `skill-remove`) is now patched to be portable for Linux/macOS by avoiding GNU-only `readlink -f`, `find -printf`, `xargs -r`, and Bash-4-only `mapfile`; mac bootstrap uses `install-macos.sh` plus `config/zshrc.tools.sh`.
