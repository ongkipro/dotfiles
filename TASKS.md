...
- **Requirement:** REQ-PUBLIC-UI-REFERENCE-FIDELITY (2026-09-18 user report: Astro output can lose supplied-reference character behind generic heroes/cards).
- **Risk Level:** R2.
- **Allowed Paths:** `skills/local/design-taste/{SKILL.md,references/design-discovery.md,references/design-evaluation.md}`, `skills/local/astro-development/SKILL.md`, `skills/local/ui-validation/SKILL.md`, `TASKS.md`, `.delivery/**`.
- **Canonical Contract Owners:** `design-taste` direction/reference; `astro-development` implementation; `ui-validation` rendered evidence.
- **Accepted Invariants:** user/project design evidence wins; familiar cards/centered layouts remain valid when content justifies them; no framework/library replacement or pixel-copy rule.
...
- **Runtime Evidence:** existing rules allowed reference intent to collapse into component-kit geometry because no pre-implementation composition contract was required.
- **Verification:** skill/policy checks plus the reference-fidelity behavioral case in `design-evaluation.md`; real visual claims still require browser evidence.
- **Non-Scope / Reopen:** no runtime/model routing or new UI dependency; reopen if reference-driven work can still skip/purge the accepted composition contract.
