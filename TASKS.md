...
- **The four:**
  - **`gsap-scrolltrigger`** — re-checked 2026-08-16: still present upstream at line 235. A ready-to-file issue is drafted in `skills/local/gsap-scrolltrigger/.local-fork`, unsent — filing it is an outward action and yours to make. It includes the one-line recheck command in case it lands first. Wire `.source` once upstream ships the fix and a re-diff still matches.
